...
## Correlation matrix
```{r}
train_corr <- train %>% mutate_if(is.factor,as.numeric)
...
train_xgt <- function(train_data,test_data,best_tune)
  ...
  cv  <-  xgb.cv(data = dtrain, nround = 100,
                 print_every_n = 20,
                 nthread = 4,
                 verbose = TRUE,
                 eval_metric= "auc",
  ...
  plot2 = xgb.plot.importance(importance_matrix,main="Features Importance") 
  
  out <- list("Features Importance:" = plot2,
              Predictions=pred)
...
paste0("ok",prop.table(table(train_3$Response)))
...
train_xgt <- function(train_data,best_tune)
  ...
  machine = xgboost(data= dtrain,  objective = "binary:logistic",
                    ...
                    eval.metric = "auc")
  # Cross Validation
  cv  <-  xgb.cv(data = dtrain, nround = 1, 
                 print_every_n= 10,
                 verbose = TRUE,
                 eval_metric= "auc",
                 nfold = 5, 
  ...
  plot_roc = plot(roc.curve,main="ROC Curve")
  # Importance plot
  importance_matrix <- xgb.importance(colnames(data_train), model = machine)
  plot2 = xgb.plot.importance(importance_matrix,main="Features Importance") 
